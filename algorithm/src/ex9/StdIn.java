...
/* The StdIn class provides static methods for reading strings and numbers from standard 
 * input. For uniformity across platforms, this class uses Locale.US for the locale and "UTF-8"
 * for the character-set encoding. The English language locale is consistent with the formatting 
 * conventions for Java floating-point literals, command-line arguments 
 * (via Double#parseDouble(String)) and standard output.
 * Like Scanner, reading a token also consumes preceding Java whitespace; reading a line 
 * consumes the following end-of-line delimeter; reading a character consumes nothing extra. 
 * Whitespace is defined in Character#isWhitespace(char). 
 * Newlines consist of \n, \r, \r\n, and Unicode hex code points 0x2028, 0x2029, 0x0085;
 * see <a href="http://www.docjar.com/html/api/java/util/Scanner.java.html"> Scanner.java
 * (NB: Java 6u23 and earlier uses only \r, \r, \r\n).
 * See In for a version that handles input from files, URLs, and sockets.
 * Note that Java's UTF-8 encoding does not recognize the optional byte-order mask. If the input 
 * begins with the optional byte-order mask, StdIn will have an extra character uFEFF 
 * at the beginning. See http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4508058. */
