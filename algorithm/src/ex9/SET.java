...
/* The SET class represents an ordered set of comparable keys. It supports the usual add, contains, 
 * and delete methods. It also provides ordered methods for finding the minimum, maximum, floor, 
 * and ceiling and set methods for union, intersection, and equality. Even though this implementation 
 * include the method equals(), it does not support the method hashCode() because sets are mutable.
 * This implementation uses a balanced binary search tree. It requires that the key type implements 
 * the Comparable interface and calls the compareTo() and method to compare two keys. It does not 
 * call either equals() or hashCode(). The add, contains, delete, minimum, maximum, ceiling, and floor
 * methods take logarithmic time in the worst case.
