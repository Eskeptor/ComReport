...
public class DigraphGenerator {
   ...
   /**
    ...
    public static void main(String[] args) {
        ...
        Digraph digraph = dag(V, E);

        StdOut.println("DAG");
        StdOut.println(digraph);
        StdOut.println();

        StdOut.println("DAG → DFS(0)");
        DepthFirstPaths.start(digraph, 0);
        StdOut.println();

        StdOut.println("DAG → BFS(0)");
        BreadthFirstPaths.start(digraph, 0);
        StdOut.println();

        StdOut.println("DAG → 위상정렬");
        Topological.start(digraph);
        StdOut.println();

        StdOut.println("mediumEWD.txt 파일 BellmanFord 알고리즘 테스트");
        BellmanFordSP.start(DigraphGenerator.class.getResource(".").getPath() + "mediumEWD.txt", 0);
        StdOut.println();
    }
