 ...
 * file input, URLs, and sockets. The Locale used is: language = English, country = US. 
 * This is consistent with the formatting conventions with Java floating-point literals,
 * command-line arguments (via Double#parseDouble(String)) and standard output. 
 * Like Scanner, reading a token also consumes preceding Java whitespace, reading a full 
 * line consumes the following end-of-line delimeter, while reading a character consumes
 * nothing extra. Whitespace is defined in {Character#isWhitespace(char)}. 
 * Newlines consist of \n, \r, \r\n, and Unicode hex code points 0x2028, 0x2029, 0x0085;
 * see href="http://www.docjar.com/html/api/java/util/Scanner.java.html"
...
public final class In {
    ...
    * param  socket the socket */
    ...
    /* Initializes an input stream from a given Scanner source; use with new Scanner(String)
     * to read from a string. Note that this does not create a defensive copy, so the scanner 
     ...
     * return true if this input stream is empty (except possibly whitespace); false otherwise  */
    public boolean isEmpty() {
        return !scanner.hasNext();
    }

    /* Returns true if this input stream has a next line. Use this to know whether the next call
     * to readLine() will succeed.Functionally equivalent to hasNextChar().
     * return true if this input stream has a next line; false otherwise  */
     ...
     * return true if this input stream is empty (including whitespace); false otherwise */
    ...
    /* Reads the next token from this input stream, parses it as a boolean (interpreting either 
     * "true" or "1" as true, and either "false" or "0" as false.
    ...
    /* Reads all remaining tokens from this input stream, parses them as integers, and returns them 
    ...
    /* Reads all remaining tokens from this input stream, parses them as doubles, and returns them 
     ...
     * deprecated Replaced by new In(filename).readAllInts().  */
     ...
     * deprecated Replaced by new In(filename).readAllDoubles(). */
     ...
     * deprecated Replaced by new In(filename).readAllStrings(). */
