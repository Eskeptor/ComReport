...
/*  The IndexMinPQ class represents an indexed priority queue of generic keys. It supports the 
 *  usual insert and delete-the-minimum operations, along with delete and change-the-key methods. 
 *  In order to let the client refer to keys on the priority queue, an integer between 0 and maxN-1 
 *  is associated with each key&mdash;the client uses this integer to specify which key to delete 
 *  or change. It also supports methods for peeking at the minimum key, testing if the priority 
 *  queue is empty, and iterating through the keys.
 *  This implementation uses a binary heap along with an array to associate keys with integers in 
 *  the given range. The insert, delete-the-minimum, delete, change-key, decrease-key, and 
 *  increase-key operations take logarithmic time. The is-empty, size, min-index, min-key, and 
 *  key-of operations take constant time. Construction takes time proportional to the specified capacity.
...
public class IndexMinPQ<Key extends Comparable<Key>> implements Iterable<Integer> {
    ...
    public void changeKey(int i, Key key) {
        if (i < 0 || i >= maxN) throw new IndexOutOfBoundsException();
        if (!contains(i)) throw new NoSuchElementException("index is not in the priority queue");
        keys[i] = key;  swim(qp[i]);  sink(qp[i]);
    }
    ...
    ***************************************************************************/
    ...
    private void exch(int i, int j) {
        int swap = pq[i];
        pq[i] = pq[j];   pq[j] = swap;  qp[pq[i]] = i;  qp[pq[j]] = j;
    }
