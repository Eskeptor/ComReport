...
public class PNGData {
   ...
   /*!!! optional information it absent now */
    ...
    protected int   getInt32  ( byte[] buf )  {
      int i = (ubyte(buf[0]) << 24) + (ubyte(buf[1]) << 16) + (ubyte(buf[2]) << 8)  + ubyte(buf[3]);
      return i;
    }
    
    protected int   getInt32  ( byte[] buf, int off ) {
      int i = (ubyte(buf[0+off]) << 24) + (ubyte(buf[1+off]) << 16) + (ubyte(buf[2+off]) << 8)  + ubyte(buf[3+off]);
      return i;
    }
    ...
    protected void  memCopy ( byte[] dest, int dst_off,  byte[] src,  int src_off, int count ) {
