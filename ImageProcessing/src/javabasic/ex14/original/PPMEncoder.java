...
/* Writes image data to a stream or a file encoded in PBM, PGM or PPM format, 
 * as appropriate.
 * The PBM, PGM and PPM formats are simple, popular formats for binary, greyscale and 
 * colour images on Unix systems.  This class supports only the ASCII variants of these formats; 
 * the more compact raw binary variants are not currently supported.
 *
 * Image data are written using ASCII numeric characters.  Line breaks are inserted after each row 
 * of data, and also wherever necessary to ensure that no line in the file is more than 70 characters 
 * in length.
 * PBM data are written as a stream of the characters '1' or '0'. By default, the inverted meaning 
 * of these values is used, as required by the official definition of the PBM format - with '1' signifying 
 * black and '0' signifying white.  This can be reversed by invoking the disableBitmapInversion method.
 *
 * PGM data are written as a series of ASCII decimal values, right-justified in a field four characters 
 * wide (or six characters wide in the case of 16-bit images).  PPM data are written in a band-interleaved 
 * fashion - i.e., the R, G and B values are written for each pixel in turn - with each colour 
 * component's value written as for greyscale images.  An extra space is inserted between each RGB triplet.
 *
 * Example of use:
 *     BufferedImage image =
 ...
 *     pgm.disableComments();
 *     pgm.encode(image);
 * see PPMEncoderException, PPMDecoder, java.awt.image.BufferedImage */
...
public class PPMEncoder implements ImageEncoder, PPMConstants {
    ...
    /* Maximum pixel value required in PGM and PPM headers. Default value is 255. */
  ...
  /* Constructs a PPMEncoder associated with standard output. */
    public PPMEncoder() { this(System.out); }
    ...
    public void disableComments() { comments = false; }

    /* Enables commenting of the PBM, PGM or PPM header. */
    public void enableComments() { comments = true; }

    /* Indicates whether a binary image will be inverted when written to a PBM file,
     * such that black and white are written as 1 and 0, respectively.
     * return true if a bitmap would be inverted, false otherwise.   */
    public boolean bitmapInversionEnabled() { return one == 0;  }

    /* Disables bitmap inversion when writing data in the PBM format. */
    public void disableBitmapInversion() { one = 1;  }

    /* Enables bitmap inversion when writing data in the PBM format. */
    public void enableBitmapInversion() { one = 0; }
   ...
   * Images of type BufferedImage.TYPE_BYTE_GRAY or
   * BufferedImage.TYPE_USHORT_GRAY are encoded using the
   * PGM format. Other image types are encoded using the PPM format.
   *
   * param image the image to be encoded
   * exception PPMEncoderException if the image could not be written successfully.
   ...
   * param image the image to be encoded   */
    ...
    /* Writes an RGB colour as three integer values, each right-justified in a field four characters wide.
     * The R, G and B components of the colour are packed into a single integer.
