...
public class PNGDataDecoder extends PNGData {
    ...
    public  void    readInfo    ( PNGInfo info ) throws PNGException, IOException  {
      ...
      // check signature
      if (memCompare(info.signature, png_sig, 8) != 0)  throw new PNGException("Not a PNG file");
     ...
     if (memCompare(chunk_name, png_IHDR, 4) == 0)  handleIHDR(info, length);
     else if (memCompare(chunk_name, png_PLTE, 4) == 0)  handlePLTE(info, length);
     else if (memCompare(chunk_name, png_IEND, 4) == 0)  handleIEND(info, length);
     else if (memCompare(chunk_name, png_IDAT, 4) == 0) {
       if ((mode & PNG_HAVE_IHDR) == 0) throw new PNGException("Missing IHDR before IDAT");
       else if (color_type == PNG_COLOR_TYPE_PALETTE && (mode & PNG_HAVE_PLTE) == 0)
    ...
    public  void    readRow     ( byte[] row )  throws PNGException, IOException {
      ...
      if ((flags & PNG_FLAG_ROW_INIT) == 0)  readStartRow();
      ...
      do {
        if (zstream.needsInput())  {
          ...
          zavail_in = zbuf_size;
          if (zbuf_size > idat_size)   zavail_in = idat_size;
        ...
        if (zstream.finished()) {
          if (zstream.getRemaining() != 0 || idat_size != 0)  throw new PNGException("Extra compressed data");
          mode  |= PNG_AFTER_IDAT;
    ...
    private void  handleIHDR  ( PNGInfo info, int length )  throws PNGException, IOException {
      ...
      if (mode != PNG_BEFORE_IHDR)  throw new PNGException("Out of place IHDR");
      
      /* check the length */
      if (length != 13)  throw new PNGException("Invalid IHDR chunk");
      mode |= PNG_HAVE_IHDR;
      ...
      if (interlace_type > 1)  throw new PNGException("Unknown interlace method in IHDR");
      if (compression_type > 0) throw new PNGException("Unknown compression method in IHDR");
      if (filter_type > 0)  throw new PNGException("Unknown filter method in IHDR");
      ...
      readIHDR(info, width, height, bit_depth, color_type, compression_type, filter_type, interlace_type);
    }
    ...
    private void  handlePLTE  ( PNGInfo info, int length )  throws PNGException, IOException  {
      ...
      if ((mode & PNG_HAVE_IHDR) == 0)  throw new PNGException("Missing IHDR before PLTE");
      else if ((mode & PNG_HAVE_IDAT) != 0) {
        warning("Invalid PLTE after IDAT");
        crcFinish(length);
        return;
      }
      else if ((mode & PNG_HAVE_PLTE) != 0)  throw new PNGException("Duplicate PLTE chunk");
    ...
    private void  handleUnknown ( PNGInfo info, int length )  throws PNGException, IOException {
       ...
       * callback functions for unknown chunks before they are ignored or cause an error.  */
    ...
    private void  checkChunkName ( byte[] name )  throws PNGException {
      if (name[0] < 41 || name[0] > 122  ||  (name[0] > 90 && name[0] < 97) ||
          name[1] < 41 || name[1] > 122  ||  (name[1] > 90 && name[1] < 97) ||
          name[2] < 41 || name[2] > 122  ||  (name[2] > 90 && name[2] < 97) ||
          name[3] < 41 || name[3] > 122  ||  (name[3] > 90 && name[3] < 97)) {
        throw new PNGException("Invalid chunk type");
      }
    }
    
    /*  handle method's */
    private void readIHDR ( PNGInfo info,  int vwidth, int vheight, int vbit_depth,
                           int vcolor_type, int vcompression_type,  int vfilter_type, int vinterlace_type ) {
      info.width = vwidth;
      info.height = vheight;
      ...
      if (info.color_type == PNG_COLOR_TYPE_PALETTE) info.channels = 1;
      else if ((info.color_type & PNG_COLOR_MASK_COLOR) != 0) info.channels = 3;
      else
        info.channels = 1;
    ...
    private void readStartRow() {
      ...
      /*!!! Not implemented  initReadTransformations(); */
      if (interlaced != 0) {
        if ((transformations & PNG_INTERLACE) == 0)
          num_rows = (height + png_pass_yinc[0] - 1 - png_pass_ystart[0]) / png_pass_yinc[0];
        else
          num_rows = height;
      ...
      /* calculate the maximum bytes needed, adding a byte and a pixel  for safety sake */
      vrowbytes = ((vrowbytes * (int)max_pixel_depth + 7) >> 3) + 1 + ((max_pixel_depth + 7) >> 3);
    ...
    /*  read row methods */
    private void readFilterRow ( PNGRowInfo row_info, byte[] row, int row_off,
                                byte[] vprev_row, int prev_row_off, int vfilter )  throws PNGException {
      switch (vfilter) {
        case 0:
          break;
        case 1:
        {
          ...
          bpp = (pixel_depth + 7) / 8;
          for (i = bpp, rp = bpp, lp = 0;  i < rowbytes; i++, rp++, lp++)  {
        ...
        case 2:
        {
          ...
          for (i = 0, rp = 0, pp = 0;  i < rowbytes; i++, rp++, pp++) {
            int val = (ubyte(row[rp+row_off]) + ubyte(vprev_row[pp+prev_row_off])) & 0xff;
            row[rp+row_off] = (byte) val;
          }
          break;
        }
        case 3:
        {
          ...
          bpp = (pixel_depth + 7) / 8;
          for (i = 0, rp = 0, pp = 0;  i < bpp; i++, rp++, pp++)  {
            int val = (ubyte(row[rp+row_off]) + (ubyte(vprev_row[pp+prev_row_off]) / 2)) & 0xff;
            row[rp+row_off] = (byte) val;
          }
          for (lp = 0; i < rowbytes; i++, rp++, lp++, pp++) {
            int val = (ubyte(row[rp+row_off]) + (ubyte(vprev_row[pp+prev_row_off]) + ubyte(row[lp+row_off])) / 2) & 0xff;
            row[rp+row_off] = (byte) val;
          }
          break;
        }
        case 4:
        {
          ...
          bpp = (pixel_depth + 7) / 8;
          for (i = 0, rp = 0, pp = 0,  lp = - bpp, cp = - bpp;  i < rowbytes; i++, rp++, pp++, lp++, cp++) {
    ...
    private void readFinishRow ()  throws PNGException, IOException {
      ...
      if ((flags & PNG_FLAG_ZLIB_FINISHED) == 0) {
        ...
        do {
          ...
          if (zstream.getRemaining() == 0)  throw new PNGException("Extra compressed data");
        } while (true);
        //!!! zstream.avail_out = 0;
      }
      
      if (idat_size != 0 || zstream.getRemaining() != 0)  throw new PNGException("Extra compression data");
      zstream.reset();
      mode |= PNG_AFTER_IDAT;
    }
    ...
    private void combineRow  ( byte[] row, int mask )  {
      if (mask == 0xff) {
        ...
        switch (row_info.pixel_depth) {
          case 1:
          {
            int sp;  int dp;  int m;  int shift;  int i;  int value;
            sp = 1;  dp = 0;  shift = 7;  m = 0x80;
            for (i = 0; i < width; i++)  {
              if ((m & mask) != 0)  {
          ...
          default:
          {
            int sp;  int dp;  int i, pixel_bytes;  byte m;
    ...
    private void crcRead ( byte[] buf, int length ) throws IOException {
      ...
      readData(buf, length);
      if (need_crc != 0)  calculateCRC(buf, length);
    }
    ...
    private int crcFinish ( int skip )  throws PNGException, IOException {
      ...
      for (i = skip; i > zbuf_size; i -= zbuf_size) {
        ...
        if (need_crc != 0)  calculateCRC(zbuf, zbuf_size);
      }
      if (i != 0) {
        readData(zbuf, i);
        if (need_crc != 0)  calculateCRC(zbuf, i);
      }
      crc_error = crcError();
