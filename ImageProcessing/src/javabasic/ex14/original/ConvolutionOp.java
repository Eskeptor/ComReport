...
/* A class to perform convolution.  ConvolutionOp is slower than ConvolveOp, but it is more 
 * flexible.  Four different strategies for dealing with the image borders are supported (compared 
 * with two for ConvolveOp).  Also, ConvolutionOp supports separable convolution, whereby 
 * a 1D kernel is applied along the rows of an image to generate an intermediate result, and 
 * then to the columns of this intermediate result to produce the final image. Finally, 
 * ConvolutionOp handles convolution with kernels containing negative coefficients properly,
 * by convolving to temporary storage and then rescaling the stored data to lie within a 0-255 range.
 *
 * ConvolutionOp also hosts a small number of static methods that act as shortcuts for common 
 * operations such as image blurring. These static methods delegate responsibility for convolution 
 * to ConvolutionOp or to ConvolveOp, as appropriate - ensuring that convolution is done in the 
...
public class ConvolutionOp extends NeighbourhoodOp {
   ...
   * No special measures will taken to deal with image borders.
   ...
   * return blurred image.  */
    ...
    /* Computes a uniform blur of an image using the specified neighbourhood and border
     * handling strategy.
     * param image : source image
     * param w : width of neighbourhood
     * param h : height of neighbourhood
     ...
     * return blurred image.   */
    public static BufferedImage blur(BufferedImage image, int w, int h, int border) {
        Kernel kernel = new MeanKernel(w, h);
        BufferedImageOp blurOp;
        if (border == CIRCULAR_INDEXING || border == REFLECTED_INDEXING)
            blurOp =  new ConvolutionOp(kernel, border, SINGLE_PASS, NO_RESCALING);
     ...
     * param border : border handling strategy
     * return blurred image.   */
    public static BufferedImage gaussianBlur(BufferedImage image, float sigma, int borderStrategy) {
        Kernel kernel = new SeparableGaussianKernel(sigma);
        BufferedImageOp blurOp =  new ConvolutionOp(kernel, borderStrategy, SEPARABLE, NO_RESCALING);
        return blurOp.filter(image, null);
    }

    ///////////////////////////// PUBLIC METHODS /////////////////////////////
  /* Constructs a ConvolutionOp for a given kernel.  No special measures will be taken at the image borders.  
   * Convolution will be performed in a single pass, and no rescaling of output values will take place.
   * param kernel : convolution kernel */
    public ConvolutionOp(Kernel kernel) {
        this(kernel, NO_BORDER_OP, SINGLE_PASS, NO_RESCALING);
    }

  /* Constructs a ConvolutionOp with a specified kernel, border handling strategy, calculation method 
   * and rescaling strategy.
   * param kernel : convolution kernel
   * param border : border handling strategy
