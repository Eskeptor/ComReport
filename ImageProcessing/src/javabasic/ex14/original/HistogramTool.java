package javabasic.ex14.original; /***************************************************************************
 HistogramTool.java : This Java2 application reads an image from a file named on the
 command line, then computes and displays its histogram.  A tabbed display is created for
 colour images, in which the view can be switched between histograms of the red, green and
 blue bands.

 The user can query values and their corresponding frequencies of occurrence by moving
 the cursor over the histogram.

 A menu is provided, from which the user may load a new image from a file, save a histogram
 to a file or quit the application.
 ***************************************************************************/
import java.awt.*;
...
public class HistogramTool extends JFrame implements ActionListener {
    ...
    private JFileChooser fileChooser =  new JFileChooser(System.getProperty("user.dir"));  // handles selection of files
    //////////////////////////////// METHODS /////////////////////////////////
        ...
        if (histogram.getNumBands() == 0) {
            ...
            if (fileChooser.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
                try {
                    File file = fileChooser.getSelectedFile();
                    if (file.exists()) {
                        int response = JOptionPane.showConfirmDialog(this, "File will be overwritten!  Are you sure?", "File exists",
    ...
    public static void main(String[] argv) {
        ...
        try {
            ...
            HistogramTool histTool = new HistogramTool(hist, str);
            histTool.pack();
