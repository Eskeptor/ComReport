...
public class PNGDataEncoder extends PNGData {
    ...
    public void writeInfo ( PNGInfo info )  throws PNGException, IOException {
      ...
      writeIHDR(info.width, info.height,  info.bit_depth, info.color_type, info.compression_type,
    ...
    /*  writes method */
    protected void writeIHDR ( int vwidth, int vheight, int vbit_depth,  int vcolor_type, int vcompression_type,
    ...
    protected void writeStartRow ()  throws IOException {
      /* set up row buffer */
      row_buf = new byte [(((int)usr_channels * (int)usr_bit_depth * width + 7) >> 3) + 1];
      row_buf[0] = 0;
      ...
      if ((do_filter & (PNG_FILTER_AVG | PNG_FILTER_UP | PNG_FILTER_PAETH)) != 0) {
        ...
        prev_row = new byte [(((int)usr_channels *  (int)usr_bit_depth *  width + 7) >> 3) + 1];
        memSet(prev_row, 0, (((int)usr_channels * (int)usr_bit_depth *  width + 7) >> 3) + 1);
      ...
      if (interlaced != 0) {
        if ((transformations & PNG_INTERLACE) == 0) {
          ...
          usr_width = (width +  png_pass_inc[0] - 1 -  png_pass_start[0]) / png_pass_inc[0];
        }
        else {
          num_rows = height;
          usr_width = width;
    ...
    protected void writeFindFilter ( PNGRowInfo vrow_info ) throws PNGException, IOException {
      ...
      vrow_buf = 0;
      mins = 0xffffffff;
      /* the prediction method we use is to find which method provides the smallest value 
       * when summing the abs of the distances from zero using anything >= 128 as negitive numbers. */
      
      /* We don't need to test the 'no filter' case if this is the only filter that has been chosen, 
       * as it doesn't actually do anything to the data. */
      if ((do_filter & PNG_FILTER_NONE) != 0 && do_filter != PNG_FILTER_NONE) {
        int rp;
        int sum = 0;
        int i, v;
      ...
      if ((do_filter & PNG_FILTER_SUB) != 0) {
        ...
        int sum = 0;
        int i, v;
        for (i = 0, rp = 1, dp = 1; i < bpp;  i++, rp++, dp++)  {
      ...
      if ((do_filter & PNG_FILTER_UP) != 0) {
        ...
        int sum = 0;
        int i, v;
        for (i = 0, rp = 1, dp = 1,  pp = 1; i < vrow_info.rowbytes; i++, rp++, pp++, dp++) {
      ...
      if ((do_filter & PNG_FILTER_AVG) != 0) {
        ...
        int sum = 0;
        int i, v;
        for (i = 0, rp = 1, dp = 1,  pp = 1; i < bpp; i++, rp++, pp++, dp++) {
          v = (ubyte(row_buf[rp]) - (ubyte(prev_row[pp]) / 2)) & 0xff;
          avg_row[dp] = (byte) v;
          sum += (v < 128) ? v : 256 - v;
        }
        for (lp = 1; i < vrow_info.rowbytes;  i++, rp++, pp++, lp++, dp++) {
      ...
      if ((do_filter & PNG_FILTER_PAETH) != 0) {
        ...
        for (lp = 1, cp = 1; i < vrow_info.rowbytes; i++, rp++, pp++, lp++, dp++, cp++) {
      ...
      if (best_row == 0)  writeFilteredRow(row_buf);
      else if (best_row == 1) writeFilteredRow(sub_row);
      else if (best_row == 2)  writeFilteredRow(up_row);
      else if (best_row == 3)  writeFilteredRow(avg_row);
      else if (best_row == 4)  writeFilteredRow(paeth_row);
    }
      ...
      if (interlaced != 0) {
        ...
        if ((transformations & PNG_INTERLACE) != 0) {
          ...
          do {
            ...
            if (pass >= 7)
              break;
            usr_width = (width + png_pass_inc[pass] - 1 - png_pass_start[pass]) / png_pass_inc[pass];
