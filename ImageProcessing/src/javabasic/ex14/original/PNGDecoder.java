...
/* Reads image data in the PNG format from a stream or a file. This class is merely a wrapper 
 * for PNGDataDecoder. See PNGDecoderException, java.awt.image.BufferedImage */
...
public class PNGDecoder implements ImageDecoder {
   ...
   * exception IOException if there was a problem reading data from standard input.
   * exception PNGDecoderException if the data do not conform to the PNG format. */
   ...
   * exception IOException if there was a problem reading data from the stream.
   * exception PNGDecoderException if the data do not conform to the  PNG format. */
   ...
   * exception IOException if the file does not exist or there was a problem reading data from it.
   * exception PNGDecoderException if the data do not conform to the PNG format.   */
   ...
   * and TYPE_PPM, defined in the PPMConstants interface.See PPMConstants  */
  ...
  public int getWidth() { return startInfo.width; }

  /* return height of image to be decoded. */
  public int getHeight() { return startInfo.height; }

  /* return number of pixels in image to be decoded. */
  public int getNumPixels() { return startInfo.width*startInfo.height; }

  /* return bit depth (number of bits per sample) for image to be decoded. */
  public int getBitDepth() { return startInfo.bit_depth; }

  /* return number of bytes required to store one row of image data. */
  public int getRowSize() { return startInfo.rowbytes; }

  /* Reads a row of image data. The data will be read into the supplied array or, if this is null,
   * returned as a new array of bytes.  The array size required to store a single row is obtained 
   * using the getRowSize method.
   * param row byte array in which image data will be placed, or null
   * return a new array containing row data, if null was passed as a parameter; 
   * or null if an array was passed.
  ...
  /* Decodes the input data and creates an image.Currently, this method supports only 8 bits 
   * per sample and it ignores alpha channels.  It also converts paletted images into 24-bit colour
   * images.  For different behaviour, you can read an image one row at a time using readRow(byte[]) readRow
  ...
  public BufferedImage decodeAsBufferedImage()
   throws IOException, PNGDecoderException {
    try {
      if (getBitDepth() != 8) throw new PNGDecoderException("unsupported bit depth");
      BufferedImage image;
      if (isGrey())  image = readGreyscaleImage();
      else if (hasPalette()) image = readPalettedImage();
      else if (isRGB()) image = readRGBImage();
      else
   ...
   * The palette is used to create a 24-bit colour image and is then discarded.
