...
public class StandardKernel extends Kernel {
  ...
  /* Creates a StandardKernel by reading data.
   * param reader : source of kernel data
   * param normalise : flag to indicate whether coefficients should be normalised on input
   * return the kernel.   */
  public static StandardKernel createKernel(Reader reader, boolean normalise) throws IOException {
    StreamTokenizer parser = new StreamTokenizer(new BufferedReader(reader));
    parser.commentChar('#');
    int w = getInteger(parser);
    int h = getInteger(parser);
    if (w < 1 || h < 1)  throw new IOException("invalid kernel dimensions");
  ...
  public String toString() {
    return new String(getClass().getName() + ": " + getWidth() + "x" + getHeight());
  }
  ...
  public void write(Writer writer) {
    ...
    for (int k = 0; k < getHeight(); ++k) {
      for (int j = 0; j < getWidth(); ++j, ++i)
        out.print(StringTools.rightJustify((String) coefficients.elementAt(i), maxLength+1));
      out.println();
    }
    out.flush();
  }

  /* return a NumberFormat that can be used to format coefficients when printing them. */
